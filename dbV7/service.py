...
def check_url(client_id: str, url: str) -> int:
    ...
    try:
        with conn.cursor() as cursor:
            # 0) 전역 도메인 차단 먼저 확인
            host = urlparse(normalized_url).netloc.lower()  # 예: www.naver.com

            # phishing_sites 에 다음 컬럼이 있다고 가정:
            #   domain VARCHAR(255) NULL
            #   is_domain_block TINYINT(1) NOT NULL DEFAULT 0
            cursor.execute(
                """
                SELECT 1
                FROM phishing_sites
...
def upsert_ai_cache(
    ...
    try:
        with conn.cursor() as cursor:
            ...
            cursor.execute(
                sql,
                (normalized_url, url_hash, gsb_value, ai_score, ai_reason, suggested_url, now),
            )
        conn.commit()
        return True
    except Exception as e:
