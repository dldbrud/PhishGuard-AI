...
# ============================================================
# ⭐ NEW: 전역 차단 정보 조회 API
# blocked.js에서 DB에 저장된 AI 분석 이유/추천 URL 가져올 때 사용
# ============================================================
...
async def get_global_info(body: dict):
    ...
    url_hash = service._make_url_hash(normalized)
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(
                """
                SELECT ai_reason, suggested_official_url
                FROM phishing_sites
                WHERE url_hash=%s
                LIMIT 1
                """,
            ...
            if not row:
                return {"ai_reason": None, "official_url": None}

            return {
                "ai_reason": row["ai_reason"],
                "official_url": row["suggested_official_url"],
            }
    finally:
        conn.close()
